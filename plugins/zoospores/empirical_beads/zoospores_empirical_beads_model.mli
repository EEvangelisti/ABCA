...
type params = {
  empirical : Zoospores_empirical_beads_data.empirical;
  ...
  bead_source : bead_source;
      (** Absence, file-based map, or random generation of beads. *)

  bead_radius : float;
      (** Default bead radius in grid cells. *)

  zoospore_radius : float;
      (** Effective zoospore radius used to inflate obstacles during collision tests. *)
...
val parse_bead_source :
  string -> string -> int -> bead_source

val string_of_bead_source :
  bead_source -> string

val parse_collision_response :
  string -> collision_response

val string_of_collision_response :
  collision_response -> string
