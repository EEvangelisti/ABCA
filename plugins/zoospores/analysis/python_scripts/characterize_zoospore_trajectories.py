...
def source_summary(tracks):
    ...
    for source_xml in sorted(by_source):
        ...
        rows.append({
            ...
            "n_spots_retained": sum(track.n_spots for track in source_tracks),
        })
    return rows
...
def parse_args():
    ...
    if (
        args.length_filter_max_points is not None
        and args.length_filter_max_points < 2
    ):
...
def main():
    ...
    # Complete dataset retained for traceability.
    write_metric_set(
        outdir=a.outdir,
        tracks=tracks,
        dt=a.dt,
    ...
    if not a.no_length_filter:
        lengths = np.asarray(
            [track.n_spots for track in tracks],
            dtype=int,
        )
        ...
        if a.length_filter_max_points is not None:
            ...
            threshold = float(
                np.percentile(
                    lengths,
                    a.length_filter_percentile,
                )
            )
            filter_method = (
                f"percentile_{a.length_filter_percentile:g}"
            )
            criterion = (
                f"n_spots <= {threshold:g} "
                f"(P{a.length_filter_percentile:g})"
            )

        retained = [
            track for track in tracks
            if track.n_spots <= threshold
        ]
        excluded = [
            track for track in tracks
            if track.n_spots > threshold
        ...
        print(
            ...
            f"({len(retained)}/{len(tracks)} tracks retained; "
            f"{criterion})"
        )
