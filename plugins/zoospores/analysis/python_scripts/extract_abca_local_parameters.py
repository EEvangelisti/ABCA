...
1. FAST/SLOW is represented by an empirical two-state Markov chain after
   state assignment with a configurable hysteresis half-width.
...
def infer_fast_slow_threshold(values: np.ndarray, bins: int = 120) -> tuple[float, str]:
...
def assign_states_with_hysteresis(
    records: list[dict[str, float | int | str]],
    threshold: float,
    half_width: float,
) -> None:
    """Assign FAST/SLOW states independently within each trajectory."""
    if half_width < 0:
        raise ValueError("The hysteresis half-width must be non-negative.")
    ...
    for rows in group_by_track(records).values():
        ...
        current_state = "FAST" if float(rows[0]["speed"]) >= threshold else "SLOW"
        rows[0]["state"] = current_state

        previous = rows[0]
        for row in rows[1:]:
            if not adjacent(previous, row):
                current_state = (
                    "FAST" if float(row["speed"]) >= threshold else "SLOW"
                )
            else:
                speed = float(row["speed"])
                if current_state == "SLOW" and speed >= upper:
                    current_state = "FAST"
                elif current_state == "FAST" and speed < lower:
                    current_state = "SLOW"
...
def write_state_assignments_csv(
    path: Path,
    records: list[dict[str, float | int | str]],
    threshold: float,
    half_width: float,
) -> None:
    """Export the exact state sequence used to estimate model probabilities."""
    fields = [
        "track_id",
        "step_index",
        "speed",
...
def transition_probabilities(
    ...
    for source in ("FAST", "SLOW"):
        total = counts[(source, "FAST")] + counts[(source, "SLOW")]
        for target in ("FAST", "SLOW"):
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        type=float,
        default=25.0,
        help="Symmetric hysteresis half-width around the speed threshold, in micron/s.",
    )
    ...
    if args.hysteresis_half_width < 0:
        parser.error("--hysteresis-half-width must be non-negative")
    ...
    if args.threshold is None:
        threshold, threshold_method = infer_fast_slow_threshold(speeds)
    else:
        threshold = args.threshold
        threshold_method = "user-specified"

    assign_states_with_hysteresis(
        records,
        threshold=threshold,
        half_width=args.hysteresis_half_width,
    )

    write_state_assignments_csv(
        output_dir / "abca_state_assignments.csv",
        records,
        threshold=threshold,
        half_width=args.hysteresis_half_width,
    )
    ...
    fast_speeds = finite(
        float(record["speed"]) for record in records if record["state"] == "FAST"
    )
    slow_speeds = finite(
        float(record["speed"]) for record in records if record["state"] == "SLOW"
    )
    ...
    fast_durations = finite(
        float(row["duration"]) for row in episode_rows if row["state"] == "FAST"
    )
    slow_durations = finite(
        float(row["duration"]) for row in episode_rows if row["state"] == "SLOW"
    )
    fast_lengths = finite(
        float(row["length"]) for row in episode_rows if row["state"] == "FAST"
    )
    slow_lengths = finite(
        float(row["length"]) for row in episode_rows if row["state"] == "SLOW"
    )
    ...
    add(
        "1_motion_states",
        "hysteresis_half_width",
        args.hysteresis_half_width,
        "micron/s",
        "Selected symmetric half-width used for state assignment.",
    )
    add(
        "1_motion_states",
        "slow_to_fast_threshold",
        threshold + args.hysteresis_half_width,
        "micron/s",
        "A SLOW state switches to FAST only at or above this speed.",
    )
    add(
        "1_motion_states",
        "fast_to_slow_threshold",
        threshold - args.hysteresis_half_width,
        "micron/s",
        "A FAST state switches to SLOW below this speed.",
    )
    add(
        "1_motion_states",
        "initial_fast_fraction",
        fast_speeds.size / speeds.size,
        "fraction",
        "Observed occupancy used to initialize the population.",
    )
    add(
        "1_motion_states",
        "initial_slow_fraction",
        slow_speeds.size / speeds.size,
        "fraction",
        "Observed occupancy used to initialize the population.",
    )
    for name in (
    ...
    for state, durations, lengths in (
        ("fast", fast_durations, fast_lengths),
        ("slow", slow_durations, slow_lengths),
    ):
    ...
    for label, values, unit in (
        ...
        ("fast_speed", fast_speeds, "micron/s"),
        ("slow_speed", slow_speeds, "micron/s"),
    ...
    for name, condition, values, unit in (
        ("speed", "FAST", fast_speeds, "micron/s"),
        ("speed", "SLOW", slow_speeds, "micron/s"),
    ...
    payload = {
        ...
        "state_assignment": {
            "central_threshold": threshold,
            "hysteresis_half_width": args.hysteresis_half_width,
            "lower_threshold": threshold - args.hysteresis_half_width,
            "upper_threshold": threshold + args.hysteresis_half_width,
            "audit_file": "abca_state_assignments.csv",
        },
        "model_specification": {
            "state_model": "two_state_empirical_markov_chain_with_hysteresis",
    ...
    rationale_lines = [
        ...
        "FAST/SLOW MARKOV CHAIN",
        "---------------------",
        f"Central threshold: {format_number(threshold)} micron/s ({threshold_method})",
        f"Hysteresis half-width: {format_number(args.hysteresis_half_width)} micron/s",
        f"Lower threshold: {format_number(threshold - args.hysteresis_half_width)} micron/s",
        f"Upper threshold: {format_number(threshold + args.hysteresis_half_width)} micron/s",
        "Exact retained state assignments: abca_state_assignments.csv",
        f"P(SLOW->SLOW)={format_number(transitions['P_SLOW_to_SLOW'])}",
        f"P(SLOW->FAST)={format_number(transitions['P_SLOW_to_FAST'])}",
        f"P(FAST->SLOW)={format_number(transitions['P_FAST_to_SLOW'])}",
        f"P(FAST->FAST)={format_number(transitions['P_FAST_to_FAST'])}",
        "",
        "EMPIRICAL MARGINALS",
        "-------------------",
        "Full inverse empirical CDFs are written to abca_empirical_quantiles.csv.",
        f"FAST speed observations: {fast_speeds.size}",
        f"SLOW speed observations: {slow_speeds.size}",
